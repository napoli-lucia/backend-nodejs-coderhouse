...
const loginCtrl = async (req, res, next) => {
  try {
    ...
    const foundUser = await userManager.getUser(email, password);
    if (foundUser.error) {
...
const registerCtrl = async (req, res, next) => {
  try {
    ...
    const result = await userManager.addUser(first_name, last_name, email, age, password);
...
const recoverPswCtrl = async (req, res, next) => {
  try {
    ...
    const foundUser = await userManager.changePassword(email, new_password);
    if (foundUser.error) {
      return res.status(foundUser.code).json({
        status: foundUser.code,
        message: foundUser.error,
